...
def environment_factory(environment: Environment | str, task: str) -> Environment:
    ...
    raise ValueError(f"Could not resolve environment: {environment}. Available environments: {Environment.available()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", required=True, help="Environment to sample trajectories from.")
    parser.add_argument("--agent", default="SimpleAgent", help="Agent to sample trajectories with.")
    args = parser.parse_args()
    
    asyncio.run(main(args.task, args.env, args.agent, args.timeout))
