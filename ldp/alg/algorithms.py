...
async def bulk_evaluate_consensus(
...
) -> tuple[dict[TGroupKey, tuple[list[tuple[TAnswer, int]], float, float]], float]:
    """
    ...
    Returns:
        Two-tuple of:
            - Three-tuple of consensus list generated by collections.Counter.most_common
              and the mean and standard error of the consensus.
    ...
    grouped_consensus: dict[
        TGroupKey, tuple[list[tuple[TAnswer, int]], float, float]
    ] = {}

    async def index(group_key: TGroupKey, group: list[TData]) -> int:
        bins, consensus, mean, ste = await evaluate_consensus(
            group, extract_answer_fn, num_samples, rand
        )
        grouped_consensus[group_key] = bins, mean, ste
...
async def evaluate_consensus(
...
) -> tuple[list[tuple[TAnswer, int]], TAnswer, float, float]:
    """
    ...
    Returns:
        Four-tuple of consensus bins generated by collections.Counter.most_common, the
            consensus answer, and the mean and standard error of the consensus.
    """
    rand = (
        seed
    ...
    sample_acc_mean = consensus_count / num_samples
    # Use binomial standard error since we're comparing ideal vs not ideal
    sample_acc_ste = np.sqrt(sample_acc_mean * (1.0 - sample_acc_mean) / num_samples)
    return most_common, consensus_answer, sample_acc_mean, sample_acc_ste
