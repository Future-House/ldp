...
async def evaluate_consensus(
    ...
    ideal_count = 0  # Number of groups where consensus matches ideal
    ...
    for group_key, group in groups.items():
        ...
        sample_acc_ste = np.sqrt(
            sample_acc_mean * (1.0 - sample_acc_mean) / num_samples
        )
        grouped_consensus[group_key] = most_common, sample_acc_mean, sample_acc_ste
        if ideal_answer_fn != NO_IDEAL_ANSWER_FN:  # If we have an ideal
            # Assume all items in the group have the same ideal answer
            ideal_count += consensus_answer == ideal_answer_fn(group[0])
