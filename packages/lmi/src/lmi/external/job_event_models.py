...
class JobEventCreateRequest(BaseModel):
    ...
    execution_type: ExecutionType = Field(description="Either 'TRAJECTORY' or 'SESSION'")
    cost_component: CostComponent = Field(description="Cost component: 'LLM_USAGE', 'EXTERNAL_SERVICE', or 'STEP'")
    ...
    crow: str | None = Field(default=None, description="Foreign key to the crows db key name")
    ...
    input_token_count: int | None = Field(default=None, description="Input token count for LLM calls")
    completion_token_count: int | None = Field(default=None, description="Completion token count for LLM calls")
...
class JobEventUpdateRequest(BaseModel):
    ...
    completion_token_count: int | None = Field(default=None, description="Completion token count for LLM calls")
    metadata: dict[str, Any] | None = Field(default=None, description="Additional metadata")
    started_at: datetime | None = Field(default=None, description="Start time of the cost period")
    ended_at: datetime | None = Field(default=None, description="End time of the cost period")
